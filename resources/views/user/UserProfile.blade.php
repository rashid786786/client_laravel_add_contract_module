        ...
        <div class="col-md-3">
            ...
            <div class="box box-primary">
                <div class="box-body box-profile">
                    <img class="profile-user-img img-responsive img-circle" src="{{asset($user->picture != null ? 'public/'.$user->picture :'public/images/UserProfile.png')}}" alt="User profile picture" style="height: 100px; margin-bottom: 20px">

                    <h1 class="profile-username text-center no-margin" style=" margin-bottom: 20px"><strong>{{$user->name}}</strong></h1>

                    <p class="text-center clr no-padding no-margin" style=" margin-bottom: 20px"><strong>{{$user->position ? $user->position : 'Position'}}</strong></p>

                    <p class="text-center no-padding  margin-bottom" style=" margin-bottom: 20px"><strong>{{$user->roles[0]->name ? $user->roles[0]->name : 'User Role'}}</strong></p>
        ...
        <div class="col-md-9">
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    ...
                    @endif
                    @if($action == 'show')
                        <a href="{{url('/UserProfile/edit/'.$user->id)}}" id="editbtn" class="btn btn-flat btn-info text-center pull-right">{{ trans('adminlte::adminlte.button_edit') }}</a>
                    @endif
                </ul>
                <div class="tab-content">
                    <div class="active tab-pane" id="UpdateProfile">
                        <!-- Horizontal Form -->
                            ...
                            <!-- form start -->
                            <form class="form-horizontal" role="form" id="profileform" action ="{{$action == 'edit' ?  url('User/update_info/'.$user->id) :'' }}"   method="POST" enctype="multipart/form-data">
                                @csrf
                                ...
                                <div class="box-body">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10 has-feedback {{ $errors->has('name') ? 'has-error' : '' }}">
                                            <input name="name" type="text" class="form-control" id="name" placeholder="Your Name" value="{{$user->name}}">
                                            @if ($errors->has('name'))
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="email" class="form-control" id="inputEmail3" placeholder="Email" readonly value="{{$user->email}}">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10 has-feedback {{ $errors->has('position') ? 'has-error' : '' }}">
                                            <input name="position" type="text" class="form-control" id="position" placeholder="{{ trans('adminlte::adminlte.label_your_position') }}" value="{{$user->position}}">
                    ...
                    @if($action == 'edit')
                        ...
                        <form class="form-horizontal" role="form" action="{{url('User/update_password/'.$user->id)}}" method="POST" enctype="multipart/form-data">
...
@section('js')
    <script type="text/javascript">
        ...
        @endif
        @if($errors->has('current_password') or Session::has('Current_Password_Error') or $errors->has('password') or $errors->has('password_confirmation'))
